...
impl Mask {
    ...
    fn emphasis(self) -> usize {
        ((self.0 >> 5) & 0x07) as usize
    }
...
impl Ppu {
    ...
    fn palette_rgb(&self, palette_addr: u16) -> [u8; 3] {
        let mut color = self.palette[palette_index(palette_addr)] & 0x3F;

        if self.mask.grayscale() {
            color &= 0x30;
        }

        let entry = self.mask.emphasis() * 64 + color as usize;
        let offset = entry * 3;

        [
            SYSTEM_PALETTE[offset],
            SYSTEM_PALETTE[offset + 1],
            SYSTEM_PALETTE[offset + 2],
        ]
    }
...
const SYSTEM_PALETTE: &[u8; 8 * 64 * 3] = include_bytes!("../assets/2c02g.pal");
...
mod tests {
    ...
    fn output_palette_index(mask: Mask, color: u8) -> usize {
        let color = if mask.grayscale() {
            color & 0x30
        } else {
            color & 0x3F
        };

        mask.emphasis() * 64 + color as usize
    }

    #[test]
    fn grayscale_selects_the_gray_column() {
        assert_eq!(output_palette_index(Mask(0x01), 0x2A), 0x20);
    }

    #[test]
    fn emphasis_selects_one_of_eight_palettes() {
        // Bits 5 and 7 = red + blue, emphasis palette 5.
        assert_eq!(output_palette_index(Mask(0xA0), 0x2A), 5 * 64 + 0x2A);
    }
